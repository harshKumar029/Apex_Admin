...
const ManageLeads = () => {
  ...
  // Detect clicks outside popup to close
  ...
  // Helper to check if lead is expired: Expired = pending for > 2 months
  const isExpired = (lead) => {
    if (lead.status !== 'pending') return false;
    const twoMonthsAgo = new Date();
    twoMonthsAgo.setMonth(twoMonthsAgo.getMonth() - 2);
    const submissionDate = lead.submissionDate.toDate ? lead.submissionDate.toDate() : new Date(lead.submissionDate);
  ...
  const getFilteredData = (filter) => {
    switch (filter) {
      ...
      case 'Rejected':
        return leads.filter((lead) => lead.status === 'rejected');
      case 'Expired':
  ...
  const getBankIcon = (bankId) => {
    return bankIcons[bankId] || SBIBANK; // Default to SBI if not found
  };
      ...
      {showDeleteConfirmModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg">
            <h2 className="text-lg font-medium mb-4">Are you sure you want to delete this lead?</h2>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4">
        <div className="w-full sm:w-auto">
          <div className="space-x-4 overflow-x-auto whitespace-nowrap hide-scrollbar">
            {['All', 'New', 'Approved', 'Pending', 'Rejected', 'Expired'].map((filter) => (
              <button
                key={filter}
                onClick={() => {
      ...
      <div className="overflow-x-auto sm:hidden">
        <div className="flex flex-col space-y-4 mb-3">
          {currentItems.map((row, idx) => {
            const submissionDate = row.submissionDate.toDate ? row.submissionDate.toDate() : new Date(row.submissionDate);
            const bankName = banksData[row.bankId] || row.bankId;
            const serviceName = servicesData[row.serviceId] || row.serviceId;
            const displayStatus = isExpired(row) ? 'Expired' : row.status.charAt(0).toUpperCase() + row.status.slice(1);
                    ...
                    <section className=' flex gap-3'>
                      <section>
                        <p className="h-12 w-12 rounded-full font-normal text-white text-sm bg-[#063E50] flex items-center justify-center">
                          {row.customerDetails?.fullname ? row.customerDetails.fullname.slice(0,2).toUpperCase() : 'NA'}
                        </p>
                      </section>
                      <section className=' space-y-1'>
                        <p className=' font-medium text-2xl text-[#063E50]'>{row.customerDetails?.fullname || 'N/A'}</p>
                        <p className='text-[#212529]'>+91 {row.customerDetails?.mobile || 'N/A'}</p>
                        <p className='text-[#212529]'>Lead ID: {row.id}</p>
                        <p className='text-[#212529]'>Service Chosen: {serviceName}</p>
                      </section>
                    </section>
                    <section className='flex flex-col items-center'>
                      {BankIcon && <img className='w-16 mb-1' src={BankIcon} alt='bank-logo' />}
                    ...
                    <p className=' inline-flex items-center gap-3'>
                      ...
                      <button
                        className={`px-4 py-1 rounded-full 
                          ...
                          ${row.status === 'rejected' ? 'bg-[#DC35452E] text-[#DC3545]' : ''}
                    ...
                    <p className=' inline-flex items-center gap-3'>
                      ...
                      <div className="relative">
                        ...
                        {openDropdownIndex === idx && (
                          <div ref={dropdownRef} className="absolute left-0 mt-7 w-28 bg-[#ffffffec] shadow-lg rounded-md z-10">
                            <ul className="py-1">
                              <li
        ...
        <table className="min-w-full bg-white text-left">
          <thead className='text-[#063E50] font-medium text-base border-b border-[#DEE2E6]'>
            <tr>
              ...
              <th style={{ textAlign: '-webkit-center' }} className="px-4 py-2">Date of Submission</th>
            ...
            {currentItems.map((row, index) => {
              ...
              return (
                <tr key={row.id} className=' text-sm' >
                  ...
                  <td style={{ textAlign: '-webkit-center' }} className="px-4 py-2">{row.customerDetails?.fullname || 'N/A'}</td>
                  <td style={{ textAlign: '-webkit-center' }} className="px-4 py-2">{row.customerDetails?.mobile || 'N/A'}</td>
                  <td style={{ textAlign: '-webkit-center' }} className="px-4 py-2">{row.customerDetails?.email || 'N/A'}</td>
                  <td style={{ textAlign: '-webkit-center' }} className="px-4 py-2">{bankName}</td>
                  <td style={{ textAlign: '-webkit-center' }} className="px-4 py-2">{serviceName}</td>
                  <td style={{ textAlign: '-webkit-center' }} className="px-4 py-4">{format(submissionDate, 'dd/MM/yyyy')}</td>
                  ...
                  <td className="px-4 py-3 inline-flex relative items-center gap-2">
                    <div className="relative">
                      ...
                      {openDropdownIndex === index && (
                        <div ref={dropdownRef} className="absolute left-0 mt-7 w-28 bg-[#FFFFFF] shadow-lg rounded-md z-10">
                          <ul className="py-1">
                            <li
